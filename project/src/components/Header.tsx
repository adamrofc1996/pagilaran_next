...
export const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Main Header - COMPLETELY REMOVED ALL UGM REFERENCES */}
      <header className={`fixed w-full z-50 transition-all duration-500 ${
        isScrolled 
          ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 backdrop-blur-lg shadow-lg border-b border-accent-500/50' 
           : location.pathname !== '/' ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 backdrop-blur-lg shadow-lg border-b border-accent-500/50' : 'bg-transparent'
      }`} >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-start h-20 gap-x-8">
            {/* Logo - Clean PT Pagilaran branding only */}
            <Link to="/" className="flex items-center space-x-1 group items-center">
            ...
            <nav className="hidden lg:flex items-center space-x-2 ml-4">
              ...
              {/* CTA Button - Moved inside nav and adjusted spacing */}
              <Link
                to="/#footer"
